...
> Summary: `@gsid/dnd` does not support the usage of `<svg>` (`SVGElement`) for a `<Draggable />` or it's _drag handle_. You are still able to drag SVG's around using a number of different strategies listed below
...
One of the core values of `@gsid/dnd` is accessibility
