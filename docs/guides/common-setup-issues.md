...
For detectable setup issues we try to log information in the `console` for `development` builds of `@gsid/dnd`. If things are not working, first thing to do is check your `console`.
...
We go over this in our [Get started with `@gsid/dnd` course](https://egghead.io/lessons/react-move-items-between-columns-with-react-beautiful-dnd-using-ondragend) (This is using [react-beautiful-dnd](https://github.com/atlassian/react-beautiful-dnd))
