...
`@gsid/dnd` supports dragging on touch devices such as mobiles and tablets.
...
When a user presses their finger (or other input) on a `<Draggable />` we are not sure if they where intending to _tap_, _force press_, _scroll the container_ or _drag_. **As much as possible `@gsid/dnd` aims to ensure that a users default interaction experience remains unaffected**.
