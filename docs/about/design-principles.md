...
This page goes over the design and interaction thinking behind `@websiddu/dnd`.

## Foundational idea: physicality

The core design idea of `@websiddu/dnd` is physicality: we want users to feel like they are moving physical objects around
...
In `@websiddu/dnd` a dragging items impact is based on its centre of gravity — regardless of where a user grabs an item from. A dragging items impact follows similar rules to a set of scales ⚖️. Here are some rules that are followed to allow for a natural drag experience even with items of flexible height:
...
`@websiddu/dnd` relies on movement to communicate positioning. It is trying to create a system that is based on physical metaphores. Drop shadows, lines and other affordances are useful in drag and drop contexts where natural movement is not possible.
